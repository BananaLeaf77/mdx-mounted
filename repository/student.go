...
func (r *studentRepository) GetAvailableSchedulesTrial(
	...
	// ── 3. Teacher finished-class counts (1 query) ────────────────────────────
	...
	if err := r.db.WithContext(ctx).Where("id = ?", instrumentID).First(&instrument).Error; err != nil {
		return nil, fmt.Errorf("instrumen tidak ditemukan: %w", err)
	}
	isDrum := strings.EqualFold(instrument.Name, "drum") || strings.EqualFold(instrument.Name, "drums")
	...
	// ── 6. Pre-compute next class dates & time windows ───────────────────────
	...
	// ── 7. Batch: fetch ALL active bookings in the relevant date range ─────────
	// Replaces 4 per-schedule queries (countRoomUsage, IsBookedSameDayAndTime,
	// IsTeacherBusy, hasActiveBookingOnDate) with a single aggregation query.
	...
	// ── 8. Enrich each schedule using in-memory lookups ───────────────────────
...
func (r *studentRepository) BookClassTrial(
	...
	// ── 9. Mark all overlapping schedules as booked ──────────────────────────
	if err := tx.Exec(`
		UPDATE teacher_schedules SET is_booked = true
		WHERE teacher_uuid = ? AND day_of_week = ?
		  AND (start_time::time, end_time::time) OVERLAPS (?::time, ?::time)
	`, schedule.TeacherUUID, schedule.DayOfWeek, schedule.StartTime, schedule.EndTime).Error; err != nil {
		tx.Rollback()
...
func (r *studentRepository) CancelBookedClass(
	...
	// ── 8. Release overlapping schedules if no other active bookings exist ───
	if err := tx.Exec(`
...
func (r *studentRepository) BulkBookClass(
	...
	// Update all overlapping schedules for all slots booked in this bulk action.
	for _, sch := range schedules {
		if err := tx.Exec(`
			UPDATE teacher_schedules SET is_booked = true
			WHERE teacher_uuid = ? AND day_of_week = ?
			  AND (start_time::time, end_time::time) OVERLAPS (?::time, ?::time)
		`, sch.TeacherUUID, sch.DayOfWeek, sch.StartTime, sch.EndTime).Error; err != nil {
			tx.Rollback()
			return nil, fmt.Errorf("gagal memperbarui status jadwal: %w", err)
		}
	}
...
// timesOverlap returns true when two "HH:MM" half-open intervals [a,b) and [c,d) overlap.
// Mirrors PostgreSQL's (a::time, b::time) OVERLAPS (c::time, d::time) semantics.
